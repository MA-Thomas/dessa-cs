...
%D_reactant = (kB_T / (6*pi*eta*currentStokesRs(j)) ); 
...
% % 2. --------------------------------------------------------------------
% Let's say the Stokes�Einstein radii of the two product molecules 
% are R_p1 and R_p2. Each product will be placed a distance equal 
% to its radius away from the unimolecular reaction location. 
...
% Assume the Stokes-Einstein Radius adds linearly with number of 
% molecules in an assembly (simple, if unphysical).
% UPDATE: Michaelis-Menten assumes all Stokes-Einstein Radii are the same.
xyz_Global_original = xyz_Global; 

stokesR_p1 = StokesEinsteinRadius; %stokes Einstein radius of single molecule.
% we want sph2cart to produce both positive and negative z values (at
% random)
randnum = rand(1,1);
if randnum>0.5; stokesR_p1 = -stokesR_p1;end 
stokesR_p1 = stokesR_p1 + abs( normrnd(0,0.01*stokesR_p1) ); % add some noise so we don't get Da==Db
azimuth = (2*pi)*rand(1,1); %unif sampling on [0,2pi]
elevation = (pi)*rand(1,1); %unif sampling on [0,pi]

[x_prime,y_prime,z_prime] = sph2cart(azimuth,elevation,stokesR_p1);
...
% -------------------------------------------------------------------------
stokesR_pRemaining = StokesEinsteinRadius; %stokes Einstein radius of remaining assembly - Michaelis Menten.
stokesR_pRemaining = stokesR_pRemaining + abs( normrnd(0,0.01*stokesR_pRemaining) ); % add some noise so we don't get Da==Db
azimuth = (2*pi)*rand(1,1); %unif sampling on [0,2pi]
elevation = (pi)*rand(1,1); %unif sampling on [0,pi]
% we want sph2cart to produce both positive and negative z values (at
% random)
if randnum<0.5; stokesR_pRemaining = -stokesR_pRemaining;end
[x_prime,y_prime,z_prime] = sph2cart(azimuth,elevation,stokesR_pRemaining);
...
% Store the Stokes-Einstein radii
currentStokesRs(cols_inf(1)) = abs(stokesR_p1);
currentStokesRs(currentAssemblyIDs == assemID_to_break) = abs(stokesR_pRemaining);
